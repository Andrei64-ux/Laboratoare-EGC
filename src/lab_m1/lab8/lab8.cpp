...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        point_light_pos[1] = glm::vec3(0, -1, 1);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_W)) point_light_pos[0] -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_A)) point_light_pos[0] -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_S)) point_light_pos[0] += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_D)) point_light_pos[0] += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_E)) point_light_pos[0] += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_Q)) point_light_pos[0] -= up * deltaTime * speed;
